...
def get_sid():
    # Ye function har unique login session ki ID nikalega
    return session.get('_id', 'guest')
...
def firing_engine(session_key):
    ...
    # Using unique session key instead of just UID
...
def login():
    ...
    if request.method == 'POST':
        ...
        if data and str(data.get('password')) == str(pw):
            session['user'] = uid
            # Cookie ID is generated by Flask here
            sid_key = get_sid() 
