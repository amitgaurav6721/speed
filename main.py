...
async def home():
    ...
    <script>
        ...
        async function showDash() {
            ...
            if(bData && bData.text) { wall.innerHTML = `● <b>ADMIN UPDATE:</b><br><span>${bData.text}</span>`; wall.style.display = 'block'; }
            else if(pData && pData.text) { wall.innerHTML = `● <b>UPDATE:</b><br><span>${pData.text}</span>`; wall.style.display = 'block'; }
            let sel = document.getElementById('tagSel'); sel.innerHTML = '<option value="ALL">SEND ALL TAGS (DEFAULT)</option>';
        ...
        async function smartFetch() {
            ...
            if(d.IMEI_No){
                ...
                // 🛑 Strict NAN Fix - Matching Firebase 'Lon' (Capital L)
                let lat = document.getElementById('useDef').checked ? curUser.lat : (d.Lat || d.lat || "24.919211");
                let lon = document.getElementById('useDef').checked ? (curUser.Lon || curUser.lon) : (d.Lon || d.lon || "83.790586");
