...
# --- 🚀 ULTIMATE DIRECT KEY INJECTION ---
db_connected = False
try:
    # Full Key Package (Bina kisi missing field ke)
...
def fetch_data(vno: str):
    ...
    try:
        ...
        return {"found": False, "err": "QUERY_REJECTED"}
...
def init(v:str, i:str, lt:str, ln:str):
    ...
    if not firing:
        ...
        logs = [f"<span style='color:#fff'>[SYS] TARGET: {v_up}</span>"]
        try:
            ...
            logs.append(f"<span style='color:#f00'>[DB] WRITE_FAIL: {str(e)[:15]}</span>")
...
async def home():
    ...
    .box { width:420px; border:2px solid #0f0; padding:25px; background:rgba(0,10,0,0.98); border-radius:15px; box-shadow: 0 0 20px #0f0; }
    ...
    <div class="box">
        <h2 style="text-align:center;letter-spacing:4px;">NITRO V82 PRO</h2>
        <input type="text" id="v" oninput="this.value=this.value.toUpperCase()" onblur="fetchData()" placeholder="BR01XXXX">
        ...
        <div style="margin-top:10px; display:flex; justify-content:space-between; font-weight:bold;"><span>SENT: <b id="c" style="color:#fff">0</b></span><span id="st" style="color:lime">IDLE</span></div>
    </div>
    <script>
        let mon;
        function fetchData() {
            ...
            if(v.length > 4) {
                document.getElementById('log').innerHTML += `<br>[SYS] SCANNING DB FOR ${v}...`;
        ...
        function sp() { fetch('/stop'); clearInterval(mon); mon=null; document.getElementById('st').innerText="IDLE"; document.getElementById('st').style.color="lime"; }
    </script></body></html>
    """
