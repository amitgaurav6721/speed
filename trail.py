...
app = Flask(__name__)
app.secret_key = "nitro_v82_hybrid_ultimate_fixed_final_2026"
...
<style>
body{background:#000;color:#0f0;font-family:monospace;padding:5px;display:flex;flex-direction:column;align-items:center;}.strip{background:#050505;border:1px solid #0f0;width:100%;max-width:480px;padding:10px;border-radius:10px;display:flex;justify-content:space-around;font-size:11px;margin-bottom:10px;box-shadow:0 0 10px #0f0;}.header{width:100%;max-width:480px;display:flex;justify-content:space-between;padding:5px;font-size:12px;border-bottom:1px solid #333;margin-bottom:10px;}.box{border:2px solid #0f0;padding:20px;border-radius:15px;width:100%;max-width:480px;background:#050505;box-shadow:0 0 20px #0f0;}#map{height:180px;width:100%;border-radius:10px;margin-bottom:10px;border:1px solid #333;}.metric{font-size:50px;color:#fff;text-align:center;margin:10px 0;font-weight:bold;}input{width:92%;padding:12px;margin:5px 0;background:#111;border:1px solid #0f0;color:#0f0;border-radius:5px;font-weight:bold;}.btn{padding:15px;font-size:16px;width:100%;font-weight:bold;margin-top:10px;cursor:pointer;border-radius:8px;border:none;}.gps-btn{background:#00f;color:#fff;padding:10px;border-radius:5px;margin-bottom:10px;width:100%;border:none;cursor:pointer;font-weight:bold;}.start{background:#008000;color:#fff;}.stop{background:#800;color:#fff;}.reset{background:#333;color:#fff;}.preview{background:#111;color:yellow;padding:10px;font-size:10px;word-break:break-all;margin-top:10px;border:1px dashed #0f0;min-height:50px;}
</style></head>
<body>
<div class="strip"><span>TOTAL: <span id="s_total" style="color:#fff">0</span></span><span>OK: <span id="s_ok" style="color:#0f0">0</span></span><span>FAIL: <span id="s_fail" style="color:#f00">0</span></span></div>
...
<div class="box">
    ...
    <form action="/action" method="post">
        ...
        <div style="display:flex; gap:10px;"><input type="text" name="lat" id="lat" value="{{status.lat}}"><input type="text" name="lon" id="lon" value="{{status.lon}}"></div>
...
<script>
var map=L.map('map').setView([parseFloat("{{status.lat}}"), parseFloat("{{status.lon}}")], 13);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);var marker=L.marker([parseFloat("{{status.lat}}"), parseFloat("{{status.lon}}")]).addTo(map);
function getLocation(){if(navigator.geolocation){navigator.geolocation.getCurrentPosition(pos=>{let lt=pos.coords.latitude.toFixed(7);let ln=pos.coords.longitude.toFixed(7);document.getElementById('lat').value=lt;document.getElementById('lon').value=ln;marker.setLatLng([lt,ln]);map.setView([lt,ln],15);});}}
document.getElementById('vno').addEventListener('blur', function(){fetch('/check_vehicle?vno='+this.value).then(r=>r.json()).then(d=>{if(d.imei)document.getElementById('imei').value=d.imei;});});
setInterval(()=>{fetch('/data').then(r=>r.json()).then(d=>{document.getElementById('cnt').innerText=d.count;document.getElementById('preview').innerText=d.last_pkt;if(d.score){document.getElementById('s_total').innerText=d.score.total||0;document.getElementById('s_ok').innerText=d.score.ok||0;document.getElementById('s_fail').innerText=d.score.fail||0;}});},1000);
</script></body></html>
"""
...
def login():
    if request.method == 'POST':
        ...
        if u_data and str(u_data.get('password')) == str(pw):
            ...
            user_sessions[sid] = {"uid":uid, "firing":False, "count":0, "imei":"", "vno":"", "lat":str(u_data.get('lat', "25.298801")), "lon":str(u_data.get('lon', "84.651033")), "last_pkt":"Ready..."}
            return redirect(url_for('dashboard'))
    return render_template_string(LOGIN_HTML)

@app.route('/dashboard')
def dashboard():
    uid, sid = session.get('user'), session.get('device_sid')
...
def firing_engine(sid):
    ...
    while user_sessions.get(sid, {}).get("firing"):
        try:
            ...
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM); sock.sendto(payload.encode('ascii'), target)
            user_sessions[sid]["count"] += 1; sock.close(); time.sleep(0.05)
        except: time.sleep(1)
