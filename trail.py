...
app = Flask(__name__)
app.secret_key = "nitro_v82_hybrid_ultimate_fix_final_2026"
...
# --- DASHBOARD HTML (ONLY STRIP & GPS FIXED) ---
DASH_HTML = """
<!DOCTYPE html>
<html>
<head>
...
def data():
    ...
    if sid in user_sessions:
        # User Audit Fetch Fix:
        today = get_ist_time().strftime('%Y-%m-%d')
        uid = user_sessions[sid]['uid']
        # Fetching directly from the audit path seen in your screenshot
        audit_url = f"{FB_URL}/User_Audit/{today}/{uid}.json?auth={FB_SECRET}"
        score = requests.get(audit_url).json()
        user_sessions[sid]['score'] = score if score else {"total":0, "ok":0, "fail":0}
    return jsonify(user_sessions.get(sid, {}))

# (Rest of the firing and login logic same as before...)
