...
async def init(v:str, i:str, lt:str, ln:str, t:str, background_tasks: BackgroundTasks):
    if stop_event.is_set():
        ...
        # 🔥 Multi-Stream Trigger: ALL or Single Tag
        if t_up == "ALL":
...
def status(): 
    return {
        "c": engine.total_sent, 
        "f": not stop_event.is_set(),
        "db": "OK",
        "eng": "ACTIVE" if not stop_event.is_set() else "IDLE"
    }
...
async def home():
    ...
    <style>
        ...
        .chk-group input { width: 20px; height: 20px; }
        ...
        .contact-footer { text-align:center; margin-top:20px; font-size:11px; color:#444; border-top:1px solid #222; padding-top:10px; width:100%; }
        .contact-footer a { color:#0f0; text-decoration:none; }
    </style></head><body>
    ...
    <div class="login-box" id="loginScreen">
        ...
        <div class="contact-footer">Support: <a href="https://t.me/ghopghopsupport">CONTACT US</a></div>
    </div>
    ...
    <div class="dashboard" id="dashScreen" style="display:none;">
        <div class="conn-box">
            <span><span class="dot online" id="m_dot"></span>MAIN.PY</span>
            <span><span class="dot" id="e_dot"></span>ENGINE.PY</span>
            <span><span class="dot" id="d_dot"></span>DATABASE.PY</span>
        </div>
        ...
        <select id="tagSel" onchange="chkMan()">
            <option value="ALL">ALL TAGS (MULTI-STREAM)</option>
            <option value="MARK">MARK</option>
            <option value="RA18">RA18</option><option value="WTEX">WTEX</option>
            <option value="ASPL">ASPL</option><option value="LOCT14A">LOCT14A</option>
    ...
    <script>
        ...
        function showDash() { document.getElementById('loginScreen').style.display='none'; document.getElementById('dashScreen').style.display='block'; document.getElementById('dashNav').style.display='flex'; document.getElementById('u_name').innerText = curUser.mobile; initMap(); }
        ...
        setInterval(() => {
            ...
            }).catch(() => document.getElementById('m_dot').classList.remove('online'));
        }, 1000);
