...
app = Flask(__name__)
app.secret_key = "nitro_v82_hybrid_master_fixed_2026"
...
<!DOCTYPE html>
<html>
<head><title>NITRO V82 - HYBRID MASTER</title><meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" /><style>body{background:#000;color:#0f0;font-family:monospace;padding:10px;display:flex;flex-wrap:wrap;justify-content:center;gap:20px;}.score-bar{background:#050505;border:1px solid #0f0;width:100%;max-width:480px;padding:10px;border-radius:10px;display:flex;justify-content:space-around;font-size:11px;margin-bottom:-10px;box-shadow:0 0 10px #0f0;}.s-val{color:#fff;font-weight:bold;}.s-ok{color:#0f0;}.s-fail{color:#f00;}.box{border:2px solid #0f0;padding:20px;border-radius:15px;width:100%;max-width:480px;background:#050505;box-shadow:0 0 20px #0f0;}#map{height:200px;width:100%;border:1px solid #0f0;border-radius:10px;margin-bottom:15px;}.metric{font-size:55px;color:#fff;margin:5px 0;font-weight:bold;text-align:center;}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;}input{width:92%;padding:12px;background:#111;border:1px solid #0f0;color:#0f0;border-radius:5px;font-weight:bold;}.full{grid-column:span 2;}.btn{padding:15px;font-size:16px;cursor:pointer;border:none;border-radius:8px;width:100%;font-weight:bold;text-transform:uppercase;margin-top:5px;}.start{background:#008000;color:#fff;}.stop{background:#800;color:#fff;}.reset{background:#333;color:#fff;}.preview{background:#111;color:yellow;padding:12px;font-size:10px;word-break:break-all;margin-top:10px;border:1px dashed #0f0;min-height:50px;}</style></head>
<body><div class="score-bar"><span>TOTAL: <span id="s_total" class="s-val">0</span></span><span>SUCCESS: <span id="s_ok" class="s-ok">0</span></span><span>FAIL: <span id="s_fail" class="s-fail">0</span></span></div><div class="box"><div id="map"></div><div class="metric" id="cnt">0</div><form action="/action" method="post" class="grid"><div class="full"><input type="text" name="vno" id="vno" placeholder="VEHICLE NO" value="{{status.vno}}" oninput="this.value = this.value.toUpperCase();"></div><div class="full"><input type="text" name="imei" id="imei" placeholder="IMEI" value="{{status.imei}}"></div><div><input type="number" step="0.0000001" name="lat" id="lat" value="{{status.lat}}"></div><div><input type="number" step="0.0000001" name="lon" id="lon" value="{{status.lon}}"></div><div class="full"><div class="preview" id="preview">{{status.last_pkt}}</div></div><button class="btn start full" name="btn" value="start">🚀 START HYBRID ENGINE</button><button class="btn stop" name="btn" value="stop">🛑 STOP</button><button class="btn reset" name="btn" value="reset">🔄 RESET</button></form></div>
<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script><script>var map=L.map('map').setView([{{status.lat}}, {{status.lon}}], 13);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);var marker=L.marker([{{status.lat}}, {{status.lon}}]).addTo(map);document.getElementById('vno').addEventListener('blur', function(){fetch('/check_vehicle?vno='+this.value).then(r=>r.json()).then(d=>{if(d.imei)document.getElementById('imei').value=d.imei;});});setInterval(()=>{fetch('/data').then(r=>r.json()).then(d=>{document.getElementById('cnt').innerText=d.count;document.getElementById('preview').innerText=d.last_pkt;if(d.score){document.getElementById('s_total').innerText=d.score.total||0;document.getElementById('s_ok').innerText=d.score.ok||0;document.getElementById('s_fail').innerText=d.score.fail||0;}});},1000);</script></body></html>
"""
