...
<body>
    ...
    <script>
        var map = L.map('map').setView([parseFloat("{{status.lat}}"), parseFloat("{{status.lon}}")], 13);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);
        var marker = L.marker([parseFloat("{{status.lat}}"), parseFloat("{{status.lon}}")]).addTo(map);
...
@app.route('/')
def login_page():
    # Adding root route to prevent 404
    return render_template_string("PASTE_YOUR_LOGIN_HTML_HERE")

@app.route('/dashboard')
def dashboard():
    uid, sid = session.get('user'), session.get('device_sid')
    if not uid or sid not in user_sessions: return redirect(url_for('login_page'))
