...
# --- 🚀 REST API LOGIC (SPEED & AUDIT OPTIMIZED) ---
...
def init(v:str, i:str, lt:str, ln:str):
    ...
    if not firing:
        ...
        # Immediate DB Update for Data_Records
        now_ist = datetime.now(timezone.utc) + timedelta(hours=5, minutes=30)
        payload = {
            "Vehicle_No": v_up, "IMEI_No": i, "Lat": lt, "Lon": ln, 
            "Status": "Active", "Start_Time": now_ist.strftime('%H:%M:%S')
        }
    ...
    while firing:
        try:
            ...
            for _ in range(15): 
                ...
                total_sent += 1
                time.sleep(0.01) # Ultra fast delivery
            s.close()
...
# --- 🎨 FINAL UI (FULL UI + FIXED SP FUNCTION) ---
...
async def home():
    ...
    <div id="overlay"><div class="popup"><h3>📢 SYSTEM ALERT</h3><p id="bc_text"></p><button onclick="closeBC()" style="background:#ff0;color:#000;border:none;padding:10px;width:100%;cursor:pointer;font-weight:bold;">UNDERSTOOD</button></div></div>

    <div class="login-box" id="loginScreen">
        <h1 style="text-align:center;">Ghop-Ghop GPS</h1>
    ...
    <div class="dashboard" id="dashScreen">
        ...
        <div style="display:flex;gap:5px;margin-top:10px;">
            <input type="text" id="lt" placeholder="LAT"><input type="text" id="ln" placeholder="LON">
        </div>
    ...
    <script>
        ...
        async function showDash() {
            ...
            fetch(`${DB}/user_messages/${curUser.mobile}.json`).then(r=>r.json()).then(m => {
                if(m && m.text) {
                    let w = document.getElementById('u_wall');
                    w.innerHTML = `● <b>ADMIN UPDATE:</b><br><span class="wall-msg">${m.text}</span>`;
                    w.style.display = 'block';
                }
            });
        }

        function closeBC() { localStorage.setItem('last_bc_id', currentBCID); document.getElementById('overlay').style.display = 'none'; document.getElementById('dashScreen').style.filter = 'none'; }
        ...
        // --- 🚀 FINAL SP FUNCTION (NO RANDOM ID) ---
        async function sp() {
            ...
            if(parseInt(total) > 0 && v_no !== ""){
                ...
                let auditData = {
                    Vehicle_No: v_no, IMEI_No: document.getElementById('i').value,
                    Lat: document.getElementById('lt').value, Lon: document.getElementById('ln').value,
                    total_sent: total, time: ist.toTimeString().split(' ')[0], Status: "Completed"
                };
                // PUT request for specific Vehicle Key
                await fetch(`${DB}/Attack_History/${dateKey}/${curUser.mobile}/${v_no}.json`, {
                    method: 'PUT', body: JSON.stringify(auditData)
                });
            }
